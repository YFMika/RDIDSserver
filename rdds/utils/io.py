...
import torch
import os
from rdds.utils.dist import is_primary
...
def resume_best_if_possible(checkpoint_dir, model_no_ddp, optimizer):
    """
    Resume if checkpoint is available.
    Return
    - epoch of loaded checkpoint.
    ...
    last_checkpoint = os.path.join(checkpoint_dir, "checkpoint_best.pth")
    if not os.path.isfile(last_checkpoint):
        return epoch, best_val_metrics
